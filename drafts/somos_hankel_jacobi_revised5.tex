...
\begin{abstract}
Assuming that a sequence arises as the shifted Hankel transform of the coefficient sequence of a Stieltjes continued fraction, we translate a Somos-type quartic relation into a constraint on the recurrence coefficients of the associated monic orthogonal polynomials. We prove a characterization: the shifted Hankel determinants satisfy the Somos-type relation if and only if consecutive off-diagonal Jacobi coefficients satisfy a fixed symmetric biquadratic equation. Equivalently, the Somos constraint imposes an algebraic law on successive ratios of squared norms. The diagonal Jacobi coefficients do not occur in this condition. We also record an explicit quadratic algebraic generating-function ansatz whose coefficient Hankel determinants reproduce, in the accompanying symbolic computation, the Somos--4 family $A(1,t)$ with prescribed initial determinants.  
\end{abstract}
...
then form Hankel determinants.  Here we show how to generate the Somos-4 sequence directly from the generating function of moments.

Let $\lambda,\eta,\tau$ be parameters and let
\[
 G(z)=\sum_{n\geq0}g_nz^n.
\]
Define the three polynomials
\begin{align}
 A(z)={}&\lambda(\eta\tau-\eta^2)z^2
 +\bigl(-(\lambda^2\eta+2)\tau
       +\lambda^2\eta^2+2\eta-2\lambda\bigr)z
 +\lambda(\tau+\lambda-\eta),                                      \label{eq:gf-A}\\
 B(z)={}&-\lambda^2\eta\tau z^2
 +\bigl((2\lambda^3\eta+3\lambda)\tau
       -\lambda^3\eta^2-\lambda\eta+3\lambda^2\bigr)z
 +\lambda^2(\eta-2\lambda-2\tau),                                  \label{eq:gf-B}\\
 C(z)={}&-\bigl((\lambda^4\eta+\lambda^2)\tau+\lambda^3\bigr)z
 +\lambda^3(\tau+\lambda).                                         \label{eq:gf-C}
\end{align}
The generating function is the formal-power-series solution of
\begin{equation}
 A(z)G(z)^2+B(z)G(z)+C(z)=0,
 \qquad G(0)=\lambda.                                                \label{eq:gf-quadratic}
\end{equation}
Equivalently, put
\[
 D(z)=B(z)^2-4A(z)C(z)
\]
and choose the formal square root $R(z)$ by
\begin{equation}
 R(z)^2=D(z),\qquad R(0)=-\lambda^2\eta.                             \label{eq:gf-root-normalization}
\end{equation}
Then
\begin{equation}
 \boxed{\displaystyle
 G(z)=\frac{-B(z)+R(z)}{2A(z)}.}                                     \label{eq:gf-explicit}
\end{equation}
This normalization, rather than a fixed printed choice of a plus or minus
sign, is essential.  Indeed,
\[
 2A(0)\lambda+B(0)=-\lambda^2\eta,
\]
so \eqref{eq:gf-root-normalization} makes
\eqref{eq:gf-explicit} select precisely the root with constant coefficient
$g_0=\lambda$.  When the square-root routine chooses the opposite constant
term, its sign must be reversed. 
...
We find now that
\begin{equation}
 H_1=\lambda,\qquad H_2=1,\qquad H_3=1,\qquad H_4=\eta,              \label{eq:gf-initial-Hankel}
\end{equation}
and 
\begin{equation}
 H_{n+4}H_n=H_{n+3}H_{n+1}+\tau H_{n+2}^2.                           \label{eq:gf-Somos}
\end{equation}
Thus the three parameters have distinct roles: $\tau$ is the second
coefficient of the Somos family $A(1,\tau)$, whereas $\lambda$ and $\eta$
select the initial Hankel determinants $(H_1,H_2,H_3,H_4)$.

\subsection{The first shift and recovery of the Stieltjes coefficients}
The same algebraic generating function has an additional property which is
special to this construction.  Define the once-shifted Hankel determinants by
\begin{equation}
 K_n=\det(g_{i+j+1})_{0\leq i,j\leq n-1},\qquad K_0=1.
 \label{eq:gf-shifted-Hankel}
\end{equation}
Direct formal expansion of \eqref{eq:gf-quadratic} and determinant evaluation
give
\begin{equation}
 K_n=H_{n+1}\qquad(n\geq0).
 \label{eq:gf-shift-identity}
\end{equation}
Consequently, the Hankel transform of the moment sequence
$(g_1,g_2,g_3,\ldots)$ is the same Somos--4 sequence as in
\eqref{eq:gf-Somos}, with its first term removed.  This shift covariance is
not a general property of Hankel transforms.  In particular, no analogous
claim is made for the twice-shifted moments $(g_2,g_3,g_4,\ldots)$.

This observation removes the factorization freedom which is present when only
one Hankel transform is known.  To make this explicit, write the generating
function as a Stieltjes continued fraction with a leading numerator,
\begin{equation}
 G(z)=\cfrac{c_0}{1-\cfrac{c_1z}{1-\cfrac{c_2z}{1-\cfrac{c_3z}{\ddots}}}}.
...
\begin{align}
 ...
 K_n&=(c_0c_1)^n\prod_{j=1}^{n-1}(c_{2j}c_{2j+1})^{n-j}.
 \label{eq:gf-shifted-product}
\end{align}
Thus $H_1=c_0$, $H_2=c_0^2c_1c_2$, and $K_1=c_0c_1$.
Introduce the determinant ratios
\begin{equation}
 U_n=\frac{H_{n-1}H_{n+1}}{H_n^2},\qquad
 V_n=\frac{K_{n-1}K_{n+1}}{K_n^2}\qquad(n\geq1).
 \label{eq:gf-two-ratio-families}
\end{equation}
Equations \eqref{eq:gf-unshifted-product} and
\eqref{eq:gf-shifted-product} give
\begin{equation}
 U_n=c_{2n-1}c_{2n},\qquad V_n=c_{2n}c_{2n+1}.
 \label{eq:gf-adjacent-products}
\end{equation}
Knowledge of only $(H_n)$ determines the products $U_n$ but leaves one
multiplicative degree of freedom in each adjacent factorization.  Knowledge of
both $(H_n)$ and $(K_n)$ removes that freedom: starting with
\begin{equation}
 c_0=H_1,\qquad c_1=\frac{K_1}{H_1},
 \label{eq:gf-c0-c1}
\end{equation}
one obtains recursively
\begin{equation}
 c_{2n}=\frac{U_n}{c_{2n-1}},\qquad
 c_{2n+1}=\frac{V_n}{c_{2n}}.
 \label{eq:gf-c-recovery}
\end{equation}

For the algebraic generating function above,
\eqref{eq:gf-shift-identity} implies $V_n=U_{n+1}$.  Hence
\eqref{eq:gf-adjacent-products} yields
$c_{2n}=c_{2n+2}$ whenever the intervening coefficient is nonzero.  Using
\eqref{eq:gf-initial-Hankel}, one finds
\begin{equation}
 c_0=\lambda,\qquad c_{2n}=\frac1\lambda\quad(n\geq1),\qquad
 c_{2n-1}=\lambda U_n\quad(n\geq1).
 \label{eq:gf-explicit-S-coefficients}
\end{equation}
Thus the two Hankel transforms determine every Stieltjes coefficient, rather
than only the contracted products.

Finally, the Somos recurrence \eqref{eq:gf-Somos} implies the quotient
recurrence
\begin{equation}
 U_{n-1}U_n^2U_{n+1}=U_n+\tau.
 \label{eq:gf-U-recurrence}
\end{equation}
Therefore the odd Stieltjes coefficients
$r_n:=c_{2n-1}=\lambda U_n$ obey the closed recurrence
\begin{equation}
 r_{n+1}=\frac{\lambda^3(r_n+\lambda\tau)}{r_{n-1}r_n^2},
 \label{eq:gf-odd-c-recurrence}
\end{equation}
with $r_1=1/\lambda$ and $r_2=\lambda^2$.  The even coefficients remain
constant, $c_{2n}=1/\lambda$.  This gives the continued-fraction coefficients
directly from the algebraic moment generating function and its two adjacent
Hankel transforms.

The quotient expression \eqref{eq:gf-explicit} requires
$A(0)=\lambda(\tau+\lambda-\eta)\neq0$.  The branch normalization is
nondegenerate when $\lambda\eta\neq0$, because
\[
 \left.\frac{\partial}{\partial X}
 \bigl(A(z)X^2+B(z)X+C(z)\bigr)\right|_{z=0,\,X=\lambda}
 =-\lambda^2\eta.
\]
These are the admissibility conditions used implicitly by the formal-series
construction.  
