...
The transport \eqref{eq:transported-G} defines a distinguished algebraic
series over $\K$.  The preceding results prove the ordinary Hankel recurrence,
the graded companion identity, and the discriminant factorization.  They do
not prove that the coefficients of this distinguished branch belong to
\[
 \Q(\lambda,m,r,\eta,\alpha,\tau).
\]
Nor do they prove that no Hankel-compatible rationalizing transformation
exists.  Such a statement would require both a precisely specified class of
admissible transformations and an invariant obstructing descent.

A complete series-level treatment should therefore supply the explicit
transported quadratic equation, remove its common Laurent monomial and
polynomial content, select the branch by its constant term, and determine the
Galois action $q\mapsto\zeta q$ with $\zeta^6=1$ on that branch.  These are
open tasks rather than assumptions used in the results above.

\section{Conclusion}
The weighted substitution \eqref{eq:transported-G} gives an exact
six-parameter extension of the direct Somos--4 construction.  In notation
consistent with \cite{Scheuerle2026}, the parameters are
$(\lambda,m,r,\eta,\alpha,\tau)$.  The prescribed initial Hankel tuple is
unchanged, the recurrence becomes \eqref{eq:transported-recurrence}, and the
companion determinants satisfy \eqref{eq:graded-companion}.  Exact transport
of the five-parameter discriminant gives \eqref{eq:q-discriminant}; after a
coordinate rescaling, its residual genus-one curve has the rational model
\eqref{eq:rational-quartic-z}.  The remaining issue is descent of the selected
normalized formal series, not descent of the curve.
