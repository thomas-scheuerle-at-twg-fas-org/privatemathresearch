...
\]
for all relevant indices. No diagonal coefficient $\widehat d_n$ appears. Since $\widehat b_n$ is also the ratio of two consecutive squared norms, the same equation gives an algebraic law for successive norm ratios. Recent work of Liu, Wang, and Zhang~\cite{LiuWangZhang2026} studies sufficient conditions and Hankel-determinant formulae for $(\alpha,\beta)$ Somos--4 sequences. The present note instead isolates an if-and-only-if condition directly on consecutive off-diagonal Jacobi coefficients.
