...
\subsection{Quadratic convolution law for the moments}
Write
\[
 A(z)=\sum_{j=0}^{3}A_jz^j,\qquad
 B(z)=\sum_{j=0}^{3}B_jz^j,\qquad
 C(z)=\sum_{j=0}^{2}C_jz^j,
\]
and define
\[
 S_q=\sum_{k=0}^{q}g_kg_{q-k},\qquad S_q=0\quad(q<0).
\]
Coefficient extraction from \eqref{eq:gf-quadratic} gives the formal identity
\begin{equation}
 \sum_{j=0}^{3}A_jS_{q-j}
 +\sum_{j=0}^{3}B_jg_{q-j}+C_q=0,
 \label{eq:gf-convolution-identity}
\end{equation}
where $g_q=0$ for $q<0$ and $C_q=0$ for $q>2$.  This finite-width quadratic
convolution law is exact for every formal-power-series root of the algebraic
equation.

In the present double-root situation it is preferable to use
\eqref{eq:gf-desingularization}.  Put
\[
 T_q=\sum_{k=0}^{q}f_kf_{q-k},\qquad T_q=0\quad(q<0).
\]
Assuming \eqref{eq:gf-branch-compatibility}, division by $z^2$ and coefficient
extraction yield
\begin{equation}
...
\end{equation}
Here $f_q=0$ for $q<0$ and $E_j=0$ for $j>3$.  If
\begin{equation}
 2A_0m+D_1\ne0,
 \label{eq:gf-desingularized-nondegeneracy}
\end{equation}
then \eqref{eq:gf-desingularized-convolution} recursively determines the
subsequent $f_q$ from $f_0=m$. 
