...
\date{September 14, 2026 (revised)}

\begin{document}
\maketitle

\begin{abstract}
Assuming that a sequence arises as the shifted Hankel transform of the coefficient sequence of a Stieltjes continued fraction, we translate a Somos-type quartic relation into a constraint on the recurrence coefficients of the associated monic orthogonal polynomials. We prove a characterization: the shifted Hankel determinants satisfy the Somos-type relation if and only if consecutive off-diagonal Jacobi coefficients satisfy a fixed symmetric biquadratic equation. Equivalently, the Somos constraint imposes an algebraic law on successive ratios of squared norms. The diagonal Jacobi coefficients do not occur in this condition. We also give a corrected five-parameter quadratic algebraic generating function whose coefficient Hankel determinants realize the Somos--4 family $A(1,\tau)$ with prescribed initial determinants $(\lambda,m,r,\eta)$.  Its construction is derived from a paired-Hankel reconstruction of a Stieltjes fraction and a quadratic closure of the recurrent tail.  The once-shifted coefficient sequence has companion Hankel determinants $\widehat H_n=H_{n+2}$, so removal of the constant coefficient advances the Somos orbit by two determinant indices.  This is an identity of Hankel transforms, not in general an identity between independently parameterized generating functions. Finally, we place Hankel determinants, Jacobi coefficients, convergent amplitudes, and the QRT first integral in one tau-function hierarchy.  
\end{abstract}
...
\section{Direct algebraic realization of prescribed Somos--4 Hankel data}
\label{sec:direct-realization}
This part incorporates the corrected inverse construction developed in the
companion working note.  In particular, the coefficient tail has a companion
Hankel transform advanced by \emph{two} determinant indices, the Stieltjes
coefficients are recovered from paired ordinary and once-shifted Hankel data,
and the algebraic generating function is derived by quadratic closure of the
recurrent continued-fraction tail.

\subsection{Hankel determinants and Somos parameters}
...
\subsection{Behind the construction: the Stieltjes fraction}
...
\subsubsection{Paired-Hankel reconstruction}
...
\subsubsection{Quadratic closure of the recurrent tail}
...
\subsection{The algebraic generating function}
...
\begin{equation}
 ...
 \label{eq:quadratic}
\end{equation}
\subsubsection{Discriminant and residual genus-one curve}
Define
\begin{equation}
...
\subsection{The two algebraic branches}
...
\subsection{The two-step companion}
...
\subsection{A recurrence for the coefficients}
...
\subsection{Base field and nondegeneracy}
...
\subsection{Nonuniqueness of the coefficient sequence}
...
\end{equation}
\begin{proposition}[Tau-function form of the convergent amplitudes]
\label{prop:tau-amplitudes}
Let $a_0=1$ and $b_n=a_{n-1}a_{n+1}/a_n^2$. Then
\begin{equation}
 E_n=\prod_{k=1}^n b_k=\frac{a_0a_{n+1}}{a_1a_n}
 =\frac{a_{n+1}}{a_1a_n}.
 \label{eq:error-amplitude-Hankel}
\end{equation}
\end{proposition}
\begin{proof}
The product telescopes, since every interior Hankel factor cancels.
\end{proof}
\begin{corollary}
\begin{equation}
 p_4=\frac{E_{n+1}}{E_{n-1}}+p_1\left(\frac{E_{n-1}}{E_n}+\frac{E_n}{E_{n+1}}\right)+p_2\frac{E_{n-1}}{E_{n+1}}.
\end{equation}
\end{corollary}
\begin{proof}
Substitute $b_n=E_n/E_{n-1}$ in the first integral.
\end{proof}
...
\section{Unified Tau-Function Interpretation}
The preceding constructions have a common tau-function description.
\begin{theorem}[Equivalent projections]
For a nonzero sequence $(a_n)$ with $a_0=1$, put
\[
b_n=\frac{a_{n-1}a_{n+1}}{a_n^2},\qquad E_n=\frac{a_{n+1}}{a_1a_n}.
\]
The Somos--4 recurrence for $a_n$, the QRT recurrence for $b_n$, the fixed
biquadratic relation for consecutive $b_n$, and the amplitude relation
\eqref{eq:error-amplitude-Somos} are equivalent wherever all quotients exist.
For shifted Hankel determinants, $b_n$ is the off-diagonal Jacobi coefficient;
for zero-diagonal prefix convergents, $E_n$ is the leading increment.
\end{theorem}
\begin{proof}
Divide the Somos recurrence by $a_n^2$, use the conserved quantity
\eqref{eq:32}, and finally substitute $b_n=E_n/E_{n-1}$.
\end{proof}
Thus
\[
\text{Hankel tau-functions }a_n\longrightarrow b_n\longrightarrow
\text{QRT orbit and genus-one curve},\qquad
E_n=\frac{a_{n+1}}{a_1a_n},\quad b_n=\frac{E_n}{E_{n-1}}.
\]
In the inverse construction, $J$ is the organizing invariant: it is computed
from the initial Hankel data, equals the QRT first integral, and parametrizes
the residual curve $E_J$ from the algebraic discriminant. Determining an
explicit birational identification with the Jacobi biquadratic remains a
...
\end{enumerate}
\end{conjecture}
The example A377264 is particularly suggestive in this context. A first step is to determine the translation point induced by $T$ on the Weierstrass model. The paired-Hankel formulas then sharpen the second step: determine the induced map on the explicit Stieltjes coordinates in \eqref{eq:paired-reconstruction-special} and compare the resulting transformation with known Volterra and modified-Volterra Darboux maps.
...
\bibitem{Barry2010Catalan}P.~Barry,
...
\bibitem{Barry2011Narayana}P.~Barry,
...
\bibitem{Barry2012Bernstein}P.~Barry,
...
\bibitem{Barry2019Pseudo}P.~Barry,
...
\bibitem{Barry2022Conjectures}P.~Barry,
...
\bibitem{ChangHuXin2015}X.-K.~Chang, X.-B.~Hu, and G.~Xin,
...
\bibitem{ChangLiu2026}X.-K.~Chang and J.~Liu,
...
\bibitem{GesselXin2006}I.~M. Gessel and G.~Xin,
...
\bibitem{JonesThron1980}W.~B. Jones and W.~J. Thron,
...
\bibitem{OvsienkoPedon2024}V.~Ovsienko and E.~Pedon,
...
\bibitem{SulankeXin2007}R.~A. Sulanke and G.~Xin,
...
\bibitem{VanderPoortenSwart2006}A.~J. van der Poorten and C.~S. Swart,
