...
\subsection{Hankel-level shift and the parameter map}
\label{sec:gf-hankel-shift}
Let
\begin{equation}
 G_{0;\ell,m,r,n,t}(x)=\sum_{q\geq0}g_qx^q
 \label{eq:G0-definition}
\end{equation}
denote a selected formal-power-series solution of
\eqref{eq:gf-quadratic} after the substitutions
\[
 \lambda=\ell,\qquad \eta=n,\qquad \tau=t.
\]
Its ordinary Hankel determinants are denoted by
\[
 H_q=\det(g_{i+j})_{0\leq i,j\leq q-1},\qquad H_0=1.
\]
The symbolic calculation is organized so that
\begin{equation}
...
\end{equation}
Consequently the shift of the \emph{Hankel sequence} acts on the parameter
 tuple as
\begin{equation}
...
\end{equation}
This statement concerns the Hankel transforms only.  In particular, it does
not imply the generating-function identity
\[
 \frac{G_{0;\ell,m,r,n,t}(x)-g_0}{x}
 =G_{0;m,r,n,(mn+tr^2)/\ell,t}(x),
\]
Distinct moment sequences, and hence distinct generating functions, may have the same Hankel
transform.

\subsection{The square-root branches at the origin}
Write
\[
 P(z,X)=A(z)X^2+B(z)X+C(z).
\]
For the coefficients displayed in \eqref{eq:gf-A}--\eqref{eq:gf-C}, the
...
\begin{equation}
 ...
 \label{eq:gf-double-root}
\end{equation}
Thus $X=\lambda$ is a double root at the origin and
\begin{equation}
 \partial_XP(0,\lambda)=2A_0\lambda+B_0=0.
 \label{eq:gf-vanishing-branch-derivative}
\end{equation}
The ordinary simple-root form of the formal implicit-function theorem is
therefore not applicable at $z=0$.  Moreover, the signs in
\eqref{eq:gf-explicit} cannot be labeled globally as the desired and undesired
branches merely from the value at the origin: both algebraic branches coalesce
there.  A branch must instead be selected by expanding the square root and
requiring the prescribed initial coefficients, beginning with
$g_0=\lambda$ and $g_1=m$.  Depending on the parameters, rationalization may
be needed to avoid cancellation.

Set
\begin{equation}
 G(z)=\lambda+zF(z),\qquad
 F(z)=\sum_{q\geq0}f_qz^q,\qquad f_q=g_{q+1}.
 \label{eq:gf-desingularization}
\end{equation}
For
\[
...
\]
Because \eqref{eq:gf-double-root} gives $E_0=D_0=0$, an ordinary power-series
branch first requires the compatibility condition
\begin{equation}
 E_1=0.
 \label{eq:gf-branch-compatibility}
\end{equation}
The next coefficient equation is
\begin{equation}
 A_0f_0^2+D_1f_0+E_2=0.
 \label{eq:gf-first-branch-coefficient}
\end{equation}

\subsection{Desingularized quadratic recurrence for the moments}
\label{sec:gf-desingularized-recurrence}
Set
\begin{equation}
 G(z)=\lambda+zF(z),\qquad
 F(z)=\sum_{q\geq0}f_qz^q,\qquad f_q=g_{q+1}.
 \label{eq:gf-desingularized-F}
\end{equation}
With
\begin{equation}
 D_j=2\lambda A_j+B_j,\qquad
 E_j=\lambda^2A_j+\lambda B_j+C_j,
 \label{eq:gf-DE-definitions}
\end{equation}
the double-root identity at the origin gives $D_0=E_0=0$.  An ordinary
power-series branch additionally requires $E_1=0$.  After substitution into
the quadratic equation and division by $z^2$, one obtains
\begin{equation}
 A(z)F(z)^2+(D_1+D_2z+D_3z^2)F(z)+E_2+E_3z=0.
 \label{eq:gf-desingularized-quadratic}
\end{equation}
In particular, the prescribed coefficient $f_0=m$ must satisfy
\begin{equation}
 A_0m^2+D_1m+E_2=0.
 \label{eq:gf-branch-selection}
\end{equation}
Thus $m$ selects one of the two branches of the desingularized quadratic.
The branch is simple precisely when
\begin{equation}
 2A_0m+D_1\ne0.
 \label{eq:gf-desingularized-nondegeneracy}
\end{equation}
Equivalently,
\begin{equation}
 (2A_0m+D_1)^2=D_1^2-4A_0E_2,
 \label{eq:gf-branch-discriminant}
\end{equation}
so the nondegeneracy condition is exactly the nonvanishing of the
discriminant of \eqref{eq:gf-branch-selection}.

Define
\begin{equation}
 T_q=\sum_{k=0}^{q}f_kf_{q-k},\qquad T_q=0\quad(q<0).
 \label{eq:gf-Tq-definition}
\end{equation}
Coefficient extraction gives
\begin{equation}
 \sum_{j=0}^{3}A_jT_{q-j}
 +D_1f_q+D_2f_{q-1}+D_3f_{q-2}+E_{q+2}=0,
 \label{eq:gf-desingularized-convolution}
\end{equation}
where $f_q=0$ for $q<0$ and $E_j=0$ for $j>3$.  For $q\geq1$,
\begin{equation}
 T_q=2mf_q+\sum_{k=1}^{q-1}f_kf_{q-k}.
 \label{eq:gf-Tq-split}
\end{equation}
Hence, under condition \eqref{eq:gf-desingularized-nondegeneracy},
\begin{equation}
 f_q=-\frac{
 ...
 {2A_0m+D_1}.
 \label{eq:gf-explicit-moment-recurrence}
\end{equation}
This is an explicit quadratic convolution recurrence for the moments
$g_{q+1}=f_q$.  It is a consequence solely of the algebraic
generating-function equation.  No Hankel or Somos assumption is used in its
derivation.

The exceptional locus $2A_0m+D_1=0$ requires separate treatment.  On this
locus the two desingularized branches coalesce at the origin, and the
first-order coefficient equation no longer determines the next moment
uniquely.  Higher-order compatibility or a Puiseux expansion may then be
required.

Finally, because $F$ is algebraic of degree at most two, it is D-finite.
Consequently, the same moment sequence also satisfies a fixed-width linear
recurrence with polynomial coefficients in the index.  This provides an
alternative practical method for generating long moment sequences.

\subsection{Nonuniqueness of the moment representation}
The Hankel transform does not in general determine the moment sequence.  The
five-parameter algebraic generating function should therefore be viewed as a
representative of a larger equivalence class rather than as a unique moment
representation of a Somos orbit.  For example, the binomial transform
\begin{equation}
 g'_q=\sum_{k=0}^{q}\binom{q}{k}g_k
 \label{eq:gf-binomial-transform}
\end{equation}
...
functionals.  Accordingly, equality of Hankel transforms must not be replaced
by equality of generating functions without an independent argument.

No classification of these equivalent moment representations is attempted
here.  The inverse problem of describing all moment sequences realizing a
given contracted Jacobi orbit remains open.
